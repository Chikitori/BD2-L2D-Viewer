  ...
  <div class="bg-gray-900 text-white h-dvh max-h-dvh flex flex-col">
    ...
    <div class="flex flex-1 flex-col lg:flex-row h-full min-h-0 overflow-hidden">
      <div class="order-1 lg:order-none hidden lg:flex flex-col min-h-0">
        <AnimationSidebar :animations="animations" :skins="skins" :exporting="isExporting"
          :screenshotting="isScreenshotting" @select="onSelectAnimation" @reset-camera="onResetCamera"
          @screenshot="onScreenshot" @export-animation="onExportAnimation" @category-change="onCategoryChange"
          @language-change="onLanguageChange" class="lg:w-64" />
      </div>
      <main class="relative flex-1 p-2 overflow-hidden">
        <div class="absolute top-2 left-2 lg:hidden z-10 flex items-center gap-2">
          ...
          <button v-show="!showMobileControls" class="p-2" @click="store.playing = !store.playing">
            <PauseIcon v-if="store.playing" />
            <PlayIcon v-else />
          </button>
          <select v-show="!showMobileControls" v-model="store.selectedSkin" class="bg-gray-700 text-white">
            <option v-for="skin in skins" :key="skin" :value="skin">{{ skin }}</option>
          </select>
          <select v-show="!showMobileControls" v-model="store.selectedAnimation" class="bg-gray-700 text-white">
    ...
    <div v-if="showMobileControls" class="fixed inset-0 z-20 bg-gray-900 lg:hidden flex flex-col">
      <button class="absolute top-3 right-4" @click="showMobileControls = false">
        ✕
      </button>
      <div class="flex-1 flex flex-col gap-2 overflow-hidden p-2">
        <div class="flex-none">
          <AnimationSidebar :animations="animations" :skins="skins" :exporting="isExporting"
            :screenshotting="isScreenshotting" @select="onSelectAnimation" @reset-camera="onResetCamera"
            @screenshot="onScreenshot" @export-animation="onExportAnimation" />
        </div>
...
function onLanguageChange(language: LanguageSetting) {
